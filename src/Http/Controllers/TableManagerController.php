...
use ManoCode\Approval\Services\TableManagerService;
...
class TableManagerController extends AdminController
{
    ...
    public function createFlow()
    {
        return amis()->DialogAction()
            ...
            ->label('创建流程')
            ->dialog((function(){
                $body = amis()->Form()->title('创建流程');
                $body = $body->api($this->getStorePath());
                $body->body([
                    amis()->TextControl('name','表单名称')->required(),
                    amis()->TextareaControl('description')->required(false),
                    amis()->TextControl('title','功能描述')->required()->disabled(),
                    amis()->TextControl('table_name','数据表')->required()->disabled(),
                    amis()->HiddenControl('columns','字段信息')->required()->disabled(),
                ]);
                return amis()
                    ->Dialog()
//                    ->size('full')
